...
# --------------------------------------------------------------------
# --------------------------------------------------------------------
APPNAME="FIREFOX" # for installer info
appname=firefox # directory name in /userdata/system/pro/...
AppName=firefox # App.AppImage name
APPPATH=/userdata/system/pro/$appname/$AppName.AppImage
APPLINK=$(curl -s https://api.github.com/repos/srevinsaju/Firefox-Appimage/releases | jq -r '.[] | select(.tag_name == "firefox") | .assets[] | select(.name | endswith(".AppImage")) | .browser_download_url')
ORIGIN=GITHUB.COM/SREVINSAJU/FIREFOX-APPIMAGE
# --------------------------------------------------------------------
# --------------------------------------------------------------------
# --------------------------------------------------------------------
# --------------------------------------------------------------------
...
wget --tries=10 --no-check-certificate --no-cache --no-cookies -q -O $pro/$appname/extra/icon.png https://github.com/trashbus99/profork/raw/master/firefox/extra/icon.png; chmod a+x $dep/* 2>/dev/null; cd ~/
...
######################################################################
######################################################################
echo 'LD_LIBRARY_PATH=/userdata/system/pro/.dep:${LD_LIBRARY_PATH} QT_FONT_DPI=128 QT_SCALE_FACTOR=1 GDK_SCALE=1 DISPLAY=:0.0 /userdata/system/pro/firefox/firefox.AppImage ${@}' >> $launcher
######################################################################
######################################################################
######################################################################
######################################################################
...
wget -q -O $icon https://github.com/trashbus99/profork/raw/master/firefox/extra/icon.png
# //
