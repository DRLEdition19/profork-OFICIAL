...
#------------------------------------------------
  "$conty" \
          ...
          --bind / /batocera \
  bash -c 'ulimit -H -n 819200 && ulimit -S -n 819200 && sysctl -w fs.inotify.max_user_watches=8192000 vm.max_map_count=2147483642 fs.file-max=8192000 >/dev/null 2>&1 && NO_AT_BRIDGE=1 GTK_A11Y=none dbus-run-session bauh '"${@}"''
#------------------------------------------------
# batocera-mouse hide
#------------------------------------------------
