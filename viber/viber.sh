...
# --------------------------------------------------------------------
# --------------------------------------------------------------------
APPNAME="VIBER" # for installer info
appname=viber # directory name in /userdata/system/pro/...
AppName=viber # App.AppImage name
...
wget --tries=10 --no-check-certificate --no-cache --no-cookies -q -O $pro/$appname/extra/icon.png https://github.com/trashbus99/profork/raw/master/viber/extra/icon.png; chmod a+x $dep/* 2>/dev/null; cd ~/
...
######################################################################
######################################################################
echo 'su - batocera -c "LD_LIBRARY_PATH=\"/userdata/system/pro/.dep:${LD_LIBRARY_PATH}\" DISPLAY=:0.0 HOME=/userdata/system/pro/'$appname'/home XDG_CONFIG_HOME=/userdata/system/pro/'$appname'/config QT_SCALE_FACTOR=\"1.25\" GDK_SCALE=\"1.25\" /userdata/system/pro/'$appname'/'$AppName'.AppImage --no-sandbox --disable-gpu \"$1\" \"$2\" \"$3\" \"$4\" \"$5\" \"$6\" \"$7\" \"$8\" \"$9\""' >> $launcher
######################################################################
######################################################################
######################################################################
######################################################################
