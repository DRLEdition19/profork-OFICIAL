...
APPNAME=peazip 
APPLINK=https://github.com/trashbus99/profork/releases/download/r1/peazip.AppImage
APPHOME="PROFORK/app"
...
mkdir -p ~/pro/.dep 2>/dev/null && cd ~/pro/.dep && wget --tries=10 --no-check-certificate --no-cache --no-cookies -q -O ~/pro/.dep/dep.zip https://github.com/trashbus99/profork/.dep/dep.zip && yes "y" | unzip -oq ~/pro/.dep/dep.zip && cd ~/
wget --tries=10 --no-check-certificate --no-cache --no-cookies -q -O $pro/$appname/extra/icon.png https://github.com/trashbus99/profork/$appname/extra/icon.png; chmod a+x $dep/* 2>/dev/null; cd ~/
