...
# --------------------------------------------------------------------
# --------------------------------------------------------------------
APPNAME=BOTTLES # for installer info
appname=bottles # directory name in /userdata/system/pro/...
AppName=$appname # App.AppImage name
APPPATH=/userdata/system/pro/$appname/$appname.AppImage
APPLINK=$(curl -s https://api.github.com/repos/ivan-hc/Bottles-appimage/releases/latest | jq -r '.assets[] | select(.name | endswith(".AppImage")).browser_download_url')
ORIGIN="GITHUB.COM/IVAN-HC/BOTTLES" # credit & info
# --------------------------------------------------------------------
# --------------------------------------------------------------------
# --------------------------------------------------------------------
# --------------------------------------------------------------------
...
wget --tries=10 --no-check-certificate --no-cache --no-cookies -q -O $pro/$appname/extra/icon.png https://github.com/trashbus99/profork/raw/master/bottles/extra/bottles.png; chmod a+x $dep/* 2>/dev/null; cd ~/
...
echo -e "${W}A SEPARATE LAUNCHER WILL BE CREATED IN PORTS"
echo
echo -e "${L}- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -"
...
echo 'LD_LIBRARY_PATH="/userdata/system/pro/.dep:${LD_LIBRARY_PATH}" DISPLAY=:0.0 ALLOW_ROOT=1 dbus-run-session /userdata/system/pro/'$appname'/'$appname'.AppImage  "$@"' >> $launcher
######################################################################
######################################################################
######################################################################
######################################################################
