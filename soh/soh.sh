...
# Fetch the latest release ZIP URL from GitHub releases
APPLINK=$(curl -s https://api.github.com/repos/HarbourMasters/Shipwright/releases/latest | grep "browser_download_url.*Linux-Performance.zip" | cut -d '"' -f 4)
APPHOME="github.com/HarbourMasters"
...
AppName=$appname
APPPATH=/userdata/system/pro/$appname/master/$AppName.AppImage
...
mkdir $pro/$appname/master 2>/dev/null

# Pass launcher command as cookie for master function
command=$pro/$appname/master/command
...
wget --tries=10 --no-check-certificate --no-cache --no-cookies -q -O $pro/$appname/master/icon.png https://github.com/trashbus99/profork/raw/master/$appname/master/icon.png
...
cols=$($dep/tput cols)
rm -rf /userdata/system/pro/$appname/master/cols
echo $cols >> /userdata/system/pro/$appname/master/cols

# Download and extract the ZIP file
temp=/userdata/system/pro/$appname/master/downloads
rm -rf $temp 2>/dev/null
mkdir -p $temp 2>/dev/null
echo -e "${G}DOWNLOADING ZIP FILE${W}"
sleep 1
curl --progress-bar --remote-name --location "$APPLINK" --output $temp/app.zip
unzip -oq $temp/app.zip -d $temp

# Move and set permissions for the AppImage
mv $temp/soh.appimage $APPPATH
chmod a+x $APPPATH
...
function batocera-pro-installer {
    ...
    echo "$(cat /userdata/system/pro/$appname/master/command)" >> $launcher
    dos2unix $launcher
    chmod a+x $launcher
    rm /userdata/system/pro/$appname/master/command 2>/dev/null
    
    # Prepare F1 - Applications - App shortcut
    shortcut=/userdata/system/pro/$appname/master/$appname.desktop
    rm -rf $shortcut 2>/dev/null
    echo "[Desktop Entry]" >> $shortcut
    echo "Version=1.0" >> $shortcut
    echo "Icon=/userdata/system/pro/$appname/master/icon.png" >> $shortcut
    ...
    # Prepare Ports file for Ship of Harkinian
port=/userdata/system/pro/$appname/ShipOfHarkinian.sh
...
echo 'DISPLAY=:0.0 /userdata/system/pro/'$appname'/soh.AppImage' >> $port
dos2unix $port
chmod a+x $port
ports=/userdata/roms/ports
if [[ -e "$ports/ShipOfHarkinian.sh" ]]; 
then 
    cp $port "$ports/ShipOfHarkinian.sh"
else 
    cp $port "$ports/ShipOfHarkinian.sh"
fi
    ...
    pre=/userdata/system/pro/$appname/master/startup
    rm -rf $pre 2>/dev/null
    echo "#!/usr/bin/env bash" >> $pre
    echo "cp /userdata/system/pro/$appname/master/$appname.desktop /usr/share/applications/ 2>/dev/null" >> $pre
    dos2unix $pre
    chmod a+x $pre
    
    # Add prelauncher to custom.sh to run at reboot
    csh=/userdata/system/custom.sh
    if [[ -e $csh ]] && [[ "$(cat $csh | grep "/userdata/system/pro/$appname/master/startup")" = "" ]]; then
        echo -e "\n/userdata/system/pro/$appname/master/startup" >> $csh
    fi
    if [[ -e $csh ]] && [[ "$(cat $csh | grep "/userdata/system/pro/$appname/master/startup" | grep "#")" != "" ]]; then
        echo -e "\n/userdata/system/pro/$appname/master/startup" >> $csh
    fi
    if [[ -e $csh ]]; then :; else
        echo -e "\n/userdata/system/pro/$appname/master/startup" >> $csh
    ...
    line $cols '='
    echo "1" >> /userdata/system/pro/$appname/master/status 2>/dev/null
    sleep 3
...
function autostart() {
    ...
    rm -f $pcsh
    temp_file=$(mktemp)
    find $pro -type f \( -path "*/master/startup" -o -path "*/extras/startup.sh" \) > $temp_file
    echo "#!/bin/bash" > $pcsh
    sort $temp_file >> $pcsh
    rm $temp_file
    chmod a+x $pcsh
